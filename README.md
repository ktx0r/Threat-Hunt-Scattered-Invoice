...
`ConditionalAccessStatus: notApplied` was confirmed on the successful attacker session. No Conditional Access policy was evaluated or enforced. This is the root cause that made the entire attack possible.
...
This investigation confirms a complete six-stage BEC kill chain executed in under 35 minutes with **zero internal alerts** during the active attack window. The £24,500 wire transfer was stopped by an external bank fraud system. That was the only control in the entire chain that worked as intended, and it is one the organization does not own or operate.
...
The `Broke sharing inheritance` events in OneDrive are an underexamined finding. In most BEC post-incident analyses the focus stays on the email chain and the wire transfer. The SharePoint and OneDrive activity here, especially the permission changes, suggests the attacker may have been preparing for secondary data access or exfiltration beyond the immediate fraud objective. That thread deserves a dedicated follow-on investigation.
